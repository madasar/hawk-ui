...
public class HWizard extends Wizard implements INewWizard {
	...
	public boolean performFinish() {
		...
		IRunnableWithProgress op = new IRunnableWithProgress() {
			public void run(IProgressMonitor monitor) throws InvocationTargetException {
				try {
					doFinish(folder, index, plugins, monitor);
	...
	private void doFinish(
		...
		//create a new hawk index at containerName folder with name fileName
			HModel.create(indexerName, folderName, plugins);
		...
		getShell().getDisplay().asyncExec(new Runnable() {
			public void run() {
				try {
					HView view = (HView) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView("hawk.ui.view.HawkView");
					view.update();
